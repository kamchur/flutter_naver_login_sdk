...
class NaverLoginSdkPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    ...
    override fun onAttachedToEngine(flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
        methodChannel = MethodChannel(
            ...
            NaverLoginSdkConstant.channelNameMethod
        )
        methodChannel.setMethodCallHandler(this)

        eventChannel = EventChannel(
            flutterPluginBinding.binaryMessenger,
            NaverLoginSdkConstant.channelNameEvent
        )
            ...
            when (call.method) {
                ...
                NaverLoginSdkConstant.Key.authenticate -> {
                    CoroutineScope(Dispatchers.Main).launch {
                        NaverLoginSdkBridge.authenticate(otherActivity, sink = sink, r = result)
                        // result.success(null)
                    }
                }
                ...
                NaverLoginSdkConstant.Key.release -> {
                    CoroutineScope(Dispatchers.Main).launch {
                        NaverLoginSdkBridge.release(sink = sink, r = result)
                        // result.success(null)
                    }
                }
                ...
                NaverLoginSdkConstant.Key.profile -> {
                    CoroutineScope(Dispatchers.Main).launch {
                        NaverLoginSdkBridge.profile(sink = sink, r = result)
                        // result.success(null)
                    }
                }
                ...
                NaverLoginSdkConstant.Key.refresh -> {
                    CoroutineScope(Dispatchers.Main).launch {
                        NaverLoginSdkBridge.refresh(sink = sink, r = result)
                        // result.success(null)
                    }
                }
